...
foreach($group in $groups) {
    ...
    Write-Progress "Searching for function apps in resource group $group" -PercentComplete (($groupCounter / $groups.Count) * 100)
    $apps =(az functionapp list -g $group --query "[?kind=='functionapp' || kind=='functionapp,linux'].name" -o tsv)

    # Iterate the web apps
    foreach($app in $apps) {

        # Query the web app for versions
        Write-Progress "Querying function app $app"
        $appConfig = (az functionapp show -n $app -g $group --query "{java:siteConfig.javaversion,netFramework:siteConfig.netFrameworkVersion,php:siteConfig.phpVersion,python:siteConfig.pythonVersion,linux:siteConfig.linuxFxVersion}") | ConvertFrom-Json
...
Write-Output $runtimes  | ConvertTo-Json > "functionapp-runtimes-$sub.json"